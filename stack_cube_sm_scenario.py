#!/usr/bin/env python3
"""
Franka Cube Stacking & Unstacking Demo — Multi-env with state machine.

Choreographed Scenario:
- Stage 1 (0s to 20s): Stack and unstack accurately, no jitter, no freeze.
- Stage 2 (20s to 40s): Stack and unstack with strong jitter.
- Stage 3 (40s to 60s): Stack and unstack with light jitter; sometimes freezes while gripping and releases the cube.
- Stage 4 (60s to 80s): Strong chaotic jitter, waving arms, and random gripper toggles.
- At 80s, the entire scenario resets back to Stage 1.
...
class StackSM:
    ...
    def compute(self, ee_pos, ee_quat, c1, c2, c3, c1_q, c2_q, c3_q):
        ...
        if 60.0 <= self.global_time < 80.0:
            ...
            for i in range(self.N):
                # Weave target in the air randomly
                ...
                # Randomly open/close gripper to flap fingers
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            if sm.global_time >= 80.0:
                ...
                last_stage_num = 0
                print(f"[{step:5d}] Scenario complete at 80s. Resetting choreography to Stage 1.", flush=True)
