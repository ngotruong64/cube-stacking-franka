#!/usr/bin/env python3
"""
Franka Cube Stacking & Unstacking — Multi-environment with state machine.

This script implements:
1. Normal stacking of 3 cubes.
2. Unstacking them back to their original position.
3. Priority rotation alignment at hover waypoints (yaw rotation aligns before descending).
4. Automatic resets.

Usage:
    conda activate env_isaaclab
    cd /home/truongnq/Desktop/3cube
    python stack_cube_sm_roundtrip.py --num_envs 4
"""
...
parser = argparse.ArgumentParser(description="Multi-env Franka Stacking and Unstacking.")
        ...
        for i in range(self.N):
            ...
            if advance:
                if s == S.DOWN_PICK:
                    ...
                    print(f"[Env {i}] Grasping cube in phase {ph} at {self.saved_pick_pos[i].cpu().numpy()} | offset yaw = {self.grasp_offset_yaw[i]:.3f} | offset pos = {self.grasp_offset_pos[i].cpu().numpy()}", flush=True)
                elif s == S.NEXT:
                    ph = self.phase[i].item()
                    if ph < 3:
                        self.phase[i] = ph + 1
...
def main():
    ...
    cube_colors = {
        "cube_1": (0.0, 132.0 / 255.0, 1.0),  # #0084ff
        "cube_2": (1.0, 0.0, 0.0),            # #ff0000
        "cube_3": (1.0, 1.0, 0.0),            # #ffff00
    }
    ...
    obs, info = env.reset()
    print(f"[INFO] Running! Robot will stack 3 cubes, then put them back, then reset.", flush=True)
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            if force.any():
                ...
                print(f"[{step:5d}] ⏱ SM-forced reset (roundtrip complete): {force_ids.tolist()}", flush=True)
