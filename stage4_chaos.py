#!/usr/bin/env python3
"""
Stage 4 — CHAOS: Strong chaotic jitter, waving arms, and random gripper toggles.

Usage:
    conda activate env_isaaclab
    cd /home/truongnq/Desktop/3cube
    python stage4_chaos.py --num_envs 4
    python stage4_chaos.py --num_envs 4 --no-video
"""
...
parser = argparse.ArgumentParser(description="Stage 4: Chaos mode.")
...
parser.add_argument("--video_folder", type=str, default="videos/stage4_chaos")
...
class StackSM:
    """Stage 4: Chaotic jitter + arm waving + random gripper toggles."""
    ...
    def compute(self, ee_pos, ee_quat, c1, c2, c3, c1_q, c2_q, c3_q):
        ...
        # ── Stage 4: CHAOTIC ARM WAVING + RANDOM GRIPPER ──
        t_wave = self.global_time
        for i in range(self.N):
            target[i, 0] += 0.15 * math.sin(t_wave * 3.0 + i * 0.5)
            target[i, 1] += 0.15 * math.cos(t_wave * 3.5 + i * 0.7)
            target[i, 2] += 0.08 * math.sin(t_wave * 2.0 + i * 1.1)
            if (int(t_wave * 10) + i) % 15 == 0:
                gripper[i] = 1.0
            elif (int(t_wave * 10) + i) % 15 == 7:
                gripper[i] = -1.0
        ...
        delta_pos += torch.randn_like(delta_pos) * 0.30
        delta_rot += torch.randn_like(delta_rot) * 0.45

        # State transitions still run so the SM advances (robot attempts to work through chaos)
...
def main():
    ...
    print(f"[INFO] Stage 4 CHAOS | Task: {task} | num_envs: {args_cli.num_envs}", flush=True)
    if args_cli.video:
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            # Stage 4: No manual reset — let the chaos run freely
            dones = terminated | truncated
            if dones.any(): sm.reset_idx(dones.nonzero(as_tuple=False).squeeze(-1))
            step += 1
            if step == 1 or step % 100 == 0:
                print(f"[{step:5d}] t={sm.global_time:.1f}s | Stage 4 CHAOS | states={[STATE_NAMES[s] for s in sm.state.tolist()]} ph={sm.phase.tolist()}", flush=True)
