#!/usr/bin/env python3
"""
Stage 1 — NORMAL: Stack and unstack accurately, no jitter, no freeze.

Usage:
    conda activate env_isaaclab
    cd /home/truongnq/Desktop/3cube
    python stage1_normal.py --num_envs 4
    python stage1_normal.py --num_envs 4 --no-video
"""
...
parser = argparse.ArgumentParser(description="Stage 1: Normal stack/unstack.")
...
parser.add_argument("--video_folder", type=str, default="videos/stage1_normal")
...
def convert_latest_video_fps(video_folder: str, target_fps: int, capture_fps: int):
    ...
    if ffmpeg is None:
        print(f"[WARN] ffmpeg not found. Kept video at {capture_fps} fps.", flush=True)
        return
    candidates = [
    ...
    if not candidates:
        print(f"[WARN] No mp4 found in {os.path.abspath(video_folder)}.", flush=True)
        return
    ...
    print(f"[INFO] Converting {os.path.basename(src)} to {target_fps} fps.", flush=True)
    result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if result.returncode != 0:
        if os.path.exists(tmp):
            os.remove(tmp)
        print(f"[WARN] Could not convert video to {target_fps} fps.\n{result.stderr[-1200:]}", flush=True)
        return
    os.replace(tmp, src)
    print(f"[INFO] Final video: {src} ({target_fps} fps).", flush=True)
...
class StackSM:
    """Stage 1: Normal operation — no jitter, no drops."""
    ...
    def compute(self, ee_pos, ee_quat, c1, c2, c3, c1_q, c2_q, c3_q):
        ...
        for i in range(self.N):
            ...
            if s == S.REST:
                ...
                target[i] = self.saved_pick_pos[i]; gripper[i] = -1.0
            elif s == S.LIFT:
                target[i] = self.saved_pick_pos[i].clone(); target[i, 2] += self.hover; gripper[i] = -1.0
            elif s == S.ABOVE_PLACE:
                target[i] = torch.tensor([t[0], t[1], t[2] + self.hover + ch], device=self.dev) + off_w; gripper[i] = -1.0
            elif s == S.DOWN_PLACE:
                target[i] = torch.tensor([t[0], t[1], t[2] + ch + self.place_margin], device=self.dev) + off_w; gripper[i] = -1.0
            elif s == S.RELEASE:
                target[i] = torch.tensor([t[0], t[1], t[2] + ch + self.place_margin], device=self.dev) + off_w
            elif s == S.RETREAT:
                target[i] = torch.tensor([t[0], t[1], t[2] + self.hover + ch], device=self.dev) + off_w
            elif s == S.DONE:
                target[i] = ee_pos[i]

        # Stage 1: NO jitter applied
        ...
        for i in range(self.N):
            ...
            if advance:
                if s == S.DOWN_PICK:
                    ...
                    ph = self.phase[i].item()
                    if ph < 3:
                        self.phase[i] = ph + 1; self.phase_timer[i] = 0.0; self.state[i] = S.REST
                    else:
                        self.phase[i] = 4; self.phase_timer[i] = 0.0; self.state[i] = S.DONE
                    self.timer[i] = 0.0
                elif s == S.RETREAT:
                    self.state[i] = S.NEXT; self.timer[i] = 0.0
                else:
                    self.state[i] = s + 1; self.timer[i] = 0.0
    ...
    def check_cube_lost(self, ee_pos, c2_pos, c3_pos):
        is_ph0 = (self.phase == 0).unsqueeze(-1)
        is_ph1 = (self.phase == 1).unsqueeze(-1)
        is_ph2 = (self.phase == 2).unsqueeze(-1)
        is_ph3 = (self.phase == 3).unsqueeze(-1)
        target_cube_pos = is_ph0.float() * c2_pos + is_ph1.float() * c3_pos + is_ph2.float() * c3_pos + is_ph3.float() * c2_pos
        ...
        return is_carrying & (dist_ee_cube > 0.08)
...
def main():
    ...
    cube_colors = {"cube_1": (0.0, 0.233, 1.0), "cube_2": (1.0, 0.0, 0.0), "cube_3": (1.0, 1.0, 0.0)}
    for cube_name, color in cube_colors.items():
        getattr(env_cfg.scene, cube_name).spawn.visual_material = sim_utils.PreviewSurfaceCfg(diffuse_color=color, roughness=0.45)
    ...
    print(f"[INFO] Stage 1 NORMAL | Task: {task} | num_envs: {args_cli.num_envs}", flush=True)
    if args_cli.video:
        ...
        print(f"[INFO] Recording video to: {os.path.abspath(args_cli.video_folder)}", flush=True)
    else:
        env = gym.make(task, cfg=env_cfg)

    obs, info = env.reset()
    sm = StackSM(dt=env_cfg.sim.dt * env_cfg.decimation, num_envs=env.unwrapped.num_envs,
                 device=env.unwrapped.device, debug_env_logs=args_cli.debug_env_logs)
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            reset_mask = cube_dropped | cube_lost | phase_timeout | sm.force_reset_mask()
            ...
            if step == 1 or step % 100 == 0:
                ...
                ph = sm.phase.tolist()
                print(f"[{step:5d}] t={sm.global_time:.1f}s | Stage 1 NORMAL | states={sn} ph={ph}", flush=True)

            if args_cli.video and step >= args_cli.video_length:
                print(f"[{step:5d}] Video length reached.", flush=True)
                break
