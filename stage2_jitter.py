#!/usr/bin/env python3
"""
Stage 2 — STRONG JITTER: Stack and unstack with strong vibration noise.

Usage:
    conda activate env_isaaclab
    cd /home/truongnq/Desktop/3cube
    python stage2_jitter.py --num_envs 4
    python stage2_jitter.py --num_envs 4 --no-video
"""
...
parser = argparse.ArgumentParser(description="Stage 2: Strong jitter stack/unstack.")
...
parser.add_argument("--video_folder", type=str, default="videos/stage2_jitter")
...
def convert_latest_video_fps(video_folder, target_fps, capture_fps):
    ...
    if ffmpeg is None: print(f"[WARN] ffmpeg not found.", flush=True); return
...
def grasp_yaw_avoiding(pick_pos, pick_quat, obstacles_pos):
    ...
    return torch.where(min_d < 1.0e-6, cube_yaw, chosen)
...
class StackSM:
    """Stage 2: Strong jitter on all actions."""
    ...
    def compute(self, ee_pos, ee_quat, c1, c2, c3, c1_q, c2_q, c3_q):
        ...
        # ── Stage 2: STRONG JITTER ──
        delta_pos += torch.randn_like(delta_pos) * 0.08
        delta_rot += torch.randn_like(delta_rot) * 0.08
    ...
    def check_cube_lost(self, ee_pos, c2_pos, c3_pos):
        is_ph0 = (self.phase == 0).unsqueeze(-1); is_ph1 = (self.phase == 1).unsqueeze(-1)
        is_ph2 = (self.phase == 2).unsqueeze(-1); is_ph3 = (self.phase == 3).unsqueeze(-1)
        tcp = is_ph0.float()*c2_pos + is_ph1.float()*c3_pos + is_ph2.float()*c3_pos + is_ph3.float()*c2_pos
        is_carry = (self.state == S.LIFT)|(self.state == S.ABOVE_PLACE)|(self.state == S.DOWN_PLACE)
        return is_carry & (torch.norm(ee_pos - tcp, dim=-1) > 0.08)
...
def main():
    ...
    print(f"[INFO] Stage 2 STRONG JITTER | Task: {task} | num_envs: {args_cli.num_envs}", flush=True)
    if args_cli.video:
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            if step == 1 or step % 100 == 0:
                print(f"[{step:5d}] t={sm.global_time:.1f}s | Stage 2 JITTER | states={[STATE_NAMES[s] for s in sm.state.tolist()]} ph={sm.phase.tolist()}", flush=True)
